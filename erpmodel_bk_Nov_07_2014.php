...
//                }
	 ...
	 */
	function _notifybymail($emails,$subj,$message,$fromname="Support",$from='support@snapittoday.com',$cc=array(),$attachment_file="")
	{
		$this->load->library("email");
		/*
	...
	function do_invoice($list,$special_inv=false)
	{
		...
		$orders=$this->db->query("select i.pnh_id,partner_id,d.catid,d.menuid,d.brandid,t.is_pnh,i_discount,i_coup_discount,i.tax,t.is_pnh,o.transid,o.id,o.itemid,i_orgprice,i_price,t.cod,t.ship,o.quantity,sum(i.orgprice*o.quantity) as mrp_amt,sum(i.price*o.quantity) as price_amt,o.other_price,o.is_memberprice,t.franchise_id,o.billing_type 
										from king_orders o 
										join king_dealitems i on i.id=o.itemid 
										join king_transactions t on t.transid = o.transid
...
*/		
	...
	function do_prods_bulk_upload()
	{
		...
		$template=array("Product Name","SKU Code","Short Description","Category (ID)","Brand (ID)","Size","Unit of measurement","MRP","VAT %","Purchase Cost","Barcode","Is offer (0 or 1)","Is Sourceable (0 or 1)","is serial required","Group ID","Attribute data");
		...
		foreach($pos as $poid=>$po)
		{
			foreach($po as $p)
			{
				...
				if($p['rqty']!=0)
				{
					...
					$p_stk_ref_id = $this->erpm->_upd_product_stock($p['product'],$p['mrp'],$p['barcode'],$p['location'],$p['rackbin'],0,$p['rqty'],4,1,$grn,-1,'',$p['exp_date'],$p['offer_stkid'],$p['offer_note'],1);
...
,instrument_no,instrument_date,instrument_issued_bank,narration,created_on,voucher_date)
	...
	function getproductdetails($id,$vid=0)
	{
		...
		$r['thirty_day_sales_qty']=@$this->db->query("select ifnull(ceil(sum(o.quantity*l.qty)/4),0) as s from m_product_deal_link l join king_orders o on o.itemid=l.itemid where l.product_id=? and o.time>".(time()-(60*24*60*60)),$id)->row()->s;
		$r['sixty_day_sales_qty']=@$this->db->query("select ifnull(ceil(sum(o.quantity*l.qty)/8),0) as s from m_product_deal_link l join king_orders o on o.itemid=l.itemid where l.product_id=? and o.time>".(time()-(30*24*60*60)),$id)->row()->s;
...
//		$this->debug_post();
		...
		$this->erpm->pnh_sendsms($login_mobile1,"Hi $name, Welcome to StoreKing. Your Franchise account is created successfully. Happy Franchising!",$id);
		foreach($cnt_name as $i=>$cname)
		{
...
//		$this->db->insert_batch("pnh_m_device_info",$inp);
	...
	function do_pnh_offline_order()
	{
       ...
       if($mid==0) 
		...
		elseif($avail_max_credit<$d_total && !$is_prepaid_franchise)
	...
	//Alotting Loyalty Points
		...
		if($insurance['insurance_deals']!='')
		{
			...
			if($new_member == 0 && $key_member==0)
			{
				...
				elseif( $insurance['opted_insurance'] == 0 && $o_total >= MEM_MIN_ORDER_VAL && $mem_orders_ttl==0 && $is_recharge_given==0 && !match_in_list($offr_sel_type,'0,2,3'))//  && $new_member == 0
				{
		...
		else
		{
			...
			{
				if( $o_total >= MEM_MIN_ORDER_VAL && $mem_orders_ttl==0 && $is_recharge_given==0 && !match_in_list($offr_sel_type,'0,2,3'))//  && $new_member == 0
				{
					$offer_ret = $this->pnh_member_recharge($o_total,$insurance,$is_tmp_order);
				}
			}
...
order by action_date";  
	...
	function p_genid($len,$for='')
	{
		...
		if($for == 'order')
			if($this->db->query("SELECT sum(t) as t FROM(SELECT COUNT(*) AS t FROM king_orders WHERE id = ? UNION SELECT COUNT(*) AS t FROM king_tmp_orders WHERE id = ? UNION SELECT COUNT(*) AS t FROM king_user_orders WHERE id = ?)AS g",array($st,$st,$st))->row()->t) 
   ...
   function do_pnh_update_execu_account_log()
   {
	...
	function get_pnh_invreturn($return_id)
	{
		...
		$return_det_res = $this->db->query("SELECT c.transid,e.franchise_id,e.franchise_name,a.return_id,a.invoice_no,return_by,a.returned_on,IF(r.return_id,1,0) AS in_transit,r.courier,r.awb,
											FROM_UNIXTIME(r.logged_on) AS logged_on,b.name AS handled_by_name,a.status,r.status AS transit_status,r.ticket_id,s.ticket_no,r.emp_phno,r.emp_name,r.transit_mode 
												from pnh_invoice_returns a 
											LEFT JOIN king_admin b ON a.handled_by = b.id 
												join king_invoice c on c.invoice_no = a.invoice_no
		...
		if($return_det_res->num_rows())
		{
			...
			$ret_prod_list_res = $this->db->query("select a.is_packed,a.readytoship,d.franchise_id,e.franchise_name,a.is_refunded,a.is_shipped,a.is_stocked,
													a.id AS return_product_id,a.return_id,a.order_id,a.product_id,a.imei_no AS new_imei_no,
													b.product_name,qty,a.barcode,a.imei_no,condition_type,a.status,IF(r.return_id,1,0) AS in_transit,r.transit_mode,r.courier,r.awb,DATE_FORMAT(FROM_UNIXTIME(r.logged_on),'%d/%m/%Y %h:%i %p') AS logged_on,r.ticket_id,r.status AS transit_status
...
order by action_date";
	...
	function sendsms_franchise_shipments($invoices='',$d_total=0)
	{
		...
		$sql_trans = "select inv.invoice_no,a.id as orderid,e.franchise_id,a.itemid,group_concat(b.name) as itemname,concat(b.print_name,'-',b.pnh_id) as print_name,i_orgprice,login_mobile1,i_price,i_coup_discount,i_discount,sum(a.quantity) as qty,c.menuid,a.transid,f.franchise_id,f.franchise_name
							...
							,mi.first_name,mi.pnh_member_id,mi.mobile,cr.courier_name,courier_no,l.hndleby_courier_id AS courierid,l.lrno,s.awb
							...
							JOIN shipment_batch_process_invoice_link s ON s.invoice_no=inv.invoice_no
							LEFT JOIN pnh_m_manifesto_sent_log l ON l.manifesto_id = s.inv_manifesto_id
							LEFT JOIN m_courier_info cr ON cr.courier_id=l.hndleby_courier_id 
...
//			echo $this->db->last_query();die();
			...
			elseif($price_type==1)
			{
					...
					$price_det_res = $this->db->query("SELECT i.id AS itemid,i.member_price,i.mp_max_allow_qty,i.mp_frn_max_qty,i.mp_mem_max_qty,i.mp_is_offer,i.mp_offer_from,i.mp_offer_to,i.mp_offer_price,i.mp_is_big_offer,i.mp_offer_note
					...
					if($price_det['member_price'] == 0 || ((1-($price_det['member_price']/$oprice))*100) > 20 )
					{
							$output['price_type']=0;
							$output['message']="Sold Out";
							$output['desc']="Member Price not created";
