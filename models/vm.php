...
//			return true;
	...
	function email($emails,$sub,$msg,$aws=false)
	{
		$config=array();
		if(!is_array($emails))
			$emails=array($emails);
...
//		$sql="select cat.name as category,deal.dealid,deal.startdate,deal.enddate,deal.pic,brand.name from king_deals as deal join king_brands as brand on brand.id=deal.brandid join king_categories as cat on cat.id=deal.catid where deal.publish=1 and deal.enddate>".time()." order by deal.enddate asc";
	...
	function getnewproducts()
	{
		...
		{
			$new=$this->db->query("select i.name,i.url from king_deals d join king_dealitems i on i.dealid=d.dealid where ? between d.startdate and d.enddate and d.publish=1 order by d.sno desc limit 12",time())->result_array();
	...
	function getdealsforspotlight()
	{
		if(($deals=$this->pettakam->get("spotlight"))===false)
		{
			...
			$deals=array();
			foreach($menu as $m)
				$deals[]=$this->db->query("select rand() as rid,m.id,i.pic,i.name,m.url as murl,i.url,m.name as menu from king_menu m join king_deals d on (d.menuid=m.id or d.menuid2=m.id) and ".time()." between d.startdate and d.enddate and d.publish=1 join king_dealitems i on i.dealid=d.dealid and i.live=1 where m.id=? order by rid desc limit 1",$m['id'])->row_array();
				
			foreach($deals as $i=>$d)
				$deals[$i]['cats']=$this->db->query("select rand() as rid,c.name,c.url from king_deals d join king_categories sc on sc.id=d.catid join king_categories c on (c.id=sc.id or c.id=sc.type) and c.type=0 where (d.menuid=? or d.menuid2=?) and ".time()." between d.startdate and d.enddate and d.publish=1 group by c.id order by rid asc limit 4",array($d['id'],$d['id']))->result_array();
			...
			{
				$row=$this->db->query("select rand() as rid,m.id,i.pic,i.name,m.url as murl,i.url,m.name as menu from king_categories m join king_categories c on c.type=m.id join king_deals d on (d.catid=c.id) and ".time()." between d.startdate and d.enddate and d.publish=1 join king_dealitems i on i.dealid=d.dealid and i.live=1 where m.name like ? order by rid desc limit 1",$m)->row_array();
				if(!empty($row))
					$cdeals[]=$row;
			}
			
			foreach($cdeals as $i=>$d)
				$cdeals[$i]['cats']=$this->db->query("select rand() as rid,c.name,c.url from king_deals d join king_categories c on c.id=d.catid and c.type=? where ".time()." between d.startdate and d.enddate and d.publish=1 group by d.catid order by rid asc limit 4",array($d['id']))->result_array();
...
//		$sql="select deal.dealid,deal.startdate,deal.enddate,deal.pic,brand.name from king_deals as deal join king_brands as brand on brand.id=deal.brandid where deal.catid=? and deal.publish=1 and deal.enddate>".time()." order by deal.enddate asc";
	...
	function getrecentsold()
	{
		...
		{
			$ret=$this->db->query("select i.id as itemid,c.name as category,i.live,i.groupbuy,c.url as caturl,rand() as rid,i.url,i.name,i.orgprice,i.price,i.pic from king_orders o join king_dealitems i on i.id=o.itemid and i.live=1 join king_deals d on d.dealid=i.dealid and ".time()." between d.startdate and d.enddate and d.publish=1 join king_categories c on c.id=d.catid group by o.itemid order by o.time desc,rid asc limit 10")->result_array();
		...
		if(($ret=$this->pettakam->get("menu"))===false)
		{
			...
			foreach($menu as $m)
			{
				if($m['id']==GIFTCARD_MENU)
					continue;
				$cats_raw=$this->db->query("select rand() as rid,c.id,c.type,c.name,c.url from king_deals as d join king_categories as c on c.id=d.catid where (d.menuid=? or d.menuid2=?) and ? between d.startdate and d.enddate and d.publish=1 group by c.id order by rid,c.name asc limit 10",array($m['id'],$m['id'],time()))->result_array();
				$cats=array();
				$brands=array();
				$maincats=array();
				...
				$best=$this->db->query("select rand() as rid,i.name,i.pic,i.url,i.price,i.name from king_deals d join king_dealitems i on i.dealid=d.dealid and i.live=1 where ? between d.startdate and d.enddate and d.publish=1 and (d.menuid=? or d.menuid2=?) order by rid desc limit 1",array(time(),$m['id'],$m['id']))->result_array();
				...
				$ret[1][$m['id']]['mbrands']=$mcats=$this->db->query("select rand() as rid,b.id,b.name,b.url from king_deals as d join king_brands as b on b.id=d.brandid where (d.menuid=? or d.menuid2=?) and ? between d.startdate and d.enddate and d.publish=1 group by b.id order by rid asc limit 5",array($m['id'],$m['id'],time()))->result_array();
				...
				$ret[1][$m['id']]['cats_count']=$this->db->query("select distinct d.catid from king_deals as d join king_categories as c on c.id=d.catid where (d.menuid=? or d.menuid2=?) and ? between d.startdate and d.enddate and d.publish=1",array($m['id'],$m['id'],time()))->num_rows();
				$ret[1][$m['id']]['brands_count']=$this->db->query("select distinct d.brandid from king_deals as d join king_brands as c on c.id=d.brandid where (d.menuid=? or d.menuid2=?) and ? between d.startdate and d.enddate and d.publish=1",array($m['id'],$m['id'],time()))->num_rows();
...
//			$this->dbm->email($email,"Access Code for Snapittoday.com","Dear $name,<br><br>Thank you for creating an account in snapittoday.com<br>Please use the access code $code to confirm this email<br><br>Snapittoday.com");
	...
	{
		$ret=$this->db->query("select i.id,b.url as burl,b.name as brand,rand() as rid,i.url,i.name,d.pic,i.price from king_deals d join king_dealitems i on i.dealid=d.dealid and i.live=1 join king_brands b on b.id=d.brandid where d.publish=1 and ".time()." between d.startdate and d.enddate and ".time()." between d.featured_start and d.featured_end order by rid desc")->result_array();
		return $ret;
	}
