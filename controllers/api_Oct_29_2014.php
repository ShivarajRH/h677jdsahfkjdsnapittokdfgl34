...
class Api extends Controller
{
			...
			if($existing_res->num_rows())
			{
				...
				if($chk_pid_exist==1)//If yes
				{
					...
					if($pids)
					{
						...
						//get catid for a item need to insert
					$chk_catid=$this->db->query("select catid from king_dealitems a join king_deals b on b.dealid=a.dealid where a.pnh_id=?",$pid)->row()->catid;
						...
						if($existing_catids->num_rows())
						{
							foreach($existing_catids->result_array() as $c)
							{
								...
								if(in_array($chk_catid, $catid_arr))
					{
									if(($c['c']+1)<=2)
									{
										$add_to_cart=1;
					...
					if($member_price_det['status'] == 'success')
					{
						...
						if($member_price_det['price_type'])
						{
							...
							if($mp_is_loyalty_point == '1') {
								$itemdet['price']= $price = $member_price_det['offer_price'];
							}
						}else
						{
							$itemdet['price']=($member_price_det['price']);
	...
	function create_order()
	{
		...
		$this->to_process_order($fid,$mid,$pid,$qty,$offr_sel_type,$redeem,0,$insurance,$d_attr,$lens_item_det,$cartid);
...
<TwilioResponse>
 <SMSMessage>
  ...
  <Uri>/v1/Accounts/snapittoday/Sms/Messages/41690593a7b8ee2f80cd08f3392d671a</Uri>
	 ...
	 */
	function to_process_order($fid=0,$mid=0,$pid=0,$qty=0,$offr_sel_type=0,$redeem='',$return=0,$insurance,$d_attr=array(),$lens_item_det=array(),$cartid='')
	{
		$min_ord = 500;
		...
		else 
		{
			...
			if($mem_det)
			{
				/*$ttl_orders=$this->db->query("SELECT SUM(l) AS ttl_orders FROM
								(SELECT COUNT(DISTINCT(transid)) AS l FROM king_orders WHERE userid=? AND STATUS NOT IN (3)
								UNION 
								SELECT COUNT(DISTINCT(transid)) AS l FROM king_tmp_orders WHERE userid=? AND approval_status NOT IN (2)
								)AS g",array($userid,$userid))->row()->ttl_orders;
...
//						$mem_orders_ttl=$ttl_orders_res->row()->l;
					...
					$is_rechrge_given = $this->erpm->chk_recharge_given($mid,$is_tmp_order);
...
//						$offr_sel_type=1;
//					}
					elseif($is_rechrge_given==0 && $key_member==0 && !match_in_list($offr_sel_type,'0,2,3') )// if recharge not given set offer type=1 (recharge)
						$offr_sel_type=1;
...
//								,'is_rechrge_given'=>@$is_rechrge_given
//								,'Insurance'=>$insurance
//							));
//					die("=================TESTING=================");
				...
				if($is_tmp_order==0)
				{
					$this->db->query("insert into king_transactions(transid,amount,paid,mode,init,actiontime,is_pnh,franchise_id,trans_created_by,batch_enabled,order_for,pnh_member_fee,api_cartid)
							values(?,?,?,?,?,?,?,?,?,?,?,?,?)"
							,array($transid,$d_total,$d_total,3,time(),time(),1,$fran['franchise_id'],$admin['userid'],$batch_enabled,$order_for,$pnh_member_fee,$cartid)) or die(mysql_error());

				}
				else 
				{
					$this->db->query("insert into king_tmp_transactions(transid,amount,paid,mode,init,actiontime,is_pnh,franchise_id,trans_created_by,batch_enabled,order_for,pnh_member_fee,api_cartid)
					values(?,?,?,?,?,?,?,?,?,?,?,?,?)"
							,array($transid,$d_total,$d_total,3,time(),time(),1,$fran['franchise_id'],$admin['userid'],$batch_enabled,$order_for,$pnh_member_fee,$cartid)) or die(mysql_error());
...
//			$title = $this->input->post('title'); 
	...
	function get_store_offers()
	{
		...
		*/ 
		...
		$store_offer_prod_res = $this->db->query("SELECT c.id,pnh_id AS pid,a.name,orgprice AS mrp,a.mp_offer_price as price,ROUND(((orgprice-member_price)/orgprice)*100) AS disc,ifnull(v.main_image,CONCAT('".IMAGES_URL."/items/small/',b.pic,'.jpg')) AS pimg_link,ifnull(v.main_image,CONCAT('".IMAGES_URL."/items/',b.pic,'.jpg')) AS main_pimg_link,a.mp_offer_to
													FROM king_dealitems a 
		...
		$big_offer_prod_res = $this->db->query("SELECT c.id,pnh_id AS pid,a.name,orgprice AS mrp,a.mp_offer_price as price,ROUND(((orgprice-member_price)/orgprice)*100) AS disc,ifnull(v.main_image,CONCAT('".IMAGES_URL."/items/small/',b.pic,'.jpg')) AS pimg_link,ifnull(v.main_image,CONCAT('".IMAGES_URL."/items/',b.pic,'.jpg')) AS main_pimg_link,a.mp_offer_to
													from king_dealitems a
													join king_deals b on a.dealid = b.dealid
													JOIN pnh_menu_group_link c ON c.menu_id = b.menuid
													LEFT JOIN m_vendor_product_images v on v.item_id=a.id
	...
	function get_upcoming_offers()
	{
		...
		$store_offer_prod_res = $this->db->query("SELECT c.id,pnh_id AS pid,a.name,orgprice AS mrp,a.member_price,a.mp_offer_price as price,ROUND(((orgprice-member_price)/orgprice)*100) AS disc,ifnull(v.main_image,CONCAT('".IMAGES_URL."/items/small/',b.pic,'.jpg')) AS pimg_link,CONCAT('".IMAGES_URL."/items/',b.pic,'.jpg') AS main_pimg_link,a.mp_offer_from,a.mp_offer_to
													FROM king_dealitems a
													JOIN king_deals b ON a.dealid = b.dealid
														JOIN pnh_menu_group_link c ON c.menu_id = b.menuid  
														LEFT JOIN m_vendor_product_images v on v.item_id=a.id
	...
	function request_franchise_otp()
	{
		...
		// validate mobno 
		if(strlen($fmobno) != 10)
			$this->_output_handle('json',false,array('error_code'=>2003,'error_msg'=>"Invalid franchise mobile number"));
		...
		else
		{
			...
			$this->erpm->pnh_sendsms($fmobno,$otp_sms,$fran_det['franchise_id'],'OTP_NO');
			
			return 	$this->_output_handle('json',true,array('status'=>'success','franchise_id'=>$fran_det['franchise_id'],'franchise_name'=>$fran_det['franchise_name'],'login_mobile'=>$fmobno,'OTP_Number'=>$otp_number,'valid_till'=>$valid_till));
		}
	}
	...
	function validate_franchise_otp()
	{
		...
		else
		{
			...
			if($is_valid_otp->num_rows())
				$this->_output_handle('json',true,array('status'=>'success','franchise_id'=>$fran_det['franchise_id'],'franchise_name'=>$fran_det['franchise_name'],'login_mobile'=>$fmobno));
			else 
	...
	//	print_r($franchise);exit;
		...
		if($franchise)
		{
			...
			{
				$this->_output_handle('json',false,array('error_code'=>2003,'error_msg'=>" Franchise type is RF"));
			}
			elseif($franchise_type==2)
			{
				$assigned_rfs=$this->db->query("select franchise_id,franchise_name from pnh_m_franchise_info where assigned_rmfid=?",$franchise_id);
				if($assigned_rfs->num_rows())
				{
					$this->_output_handle('json',true,array('status'=>'success','franchise_type'=>$franchise_type,'rural_master_fid'=>$franchise_id,'assigned_rf'=>$assigned_rfs->result_array()));
						
				}else
				 {
				 	$this->_output_handle('json',false,array('error_code'=>2003,'error_msg'=>" RF Not Linked"));
				 }
			}
		}
