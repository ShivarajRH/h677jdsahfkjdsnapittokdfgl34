...
//        echo '<pre>'.$this->db->last_query(); die();
    ...
    function jx_get_franchise_orders($franchise_id,$from=0,$to=0,$batch_type) {
        ...
        $output = $picklist_btn_msg= '';
         ...
         //if($from!=0 and $to != 0) { //$cond .= '  and tr.actiontime between '.$from.' and '.$to.' ';
         //}else //$from=$to=0;
        ...
        $rslt = $this->db->query("select DISTINCT o.*,tr.transid,tr.amount,tr.paid,tr.init,tr.is_pnh,tr.franchise_id,di.name
                                ...
                                group by o.id
                                order by tr.init desc",array($franchise_id))->result_array();//,$from,$to #,di.name
        //echo '<pre>'.$this->db->last_query(); die();
        $arr_tmp=array();
        foreach ($rslt as $i=>$row) {
                ...
                $output .= '<tr>
                         ...
                         </tr>'; //<td align="center">'.$picklist_btn_msg.'</td>
        }
        echo $output;
    }
    ...
    function manage_reservation_create_batch_form() {
        ...
        $data['pnh_terr'] = $this->db->query("select t.* from proforma_invoices pi 
                                                        ...
                                                        left join king_invoice i on o.id = i.order_id and i.invoice_status = 1
                                                        where pi.invoice_status = 1 and sd.batch_id = ? and tr.batch_enabled=1 and i.id is null
...
//        echo '<pre>';print_r($arr_out);die();
     ...
     */
    function jx_get_batched_trans_orders_list($batch_id) {
        $this->erpm->auth();
