...
<div class="container">
	<h2>Newly Added Top Sold Products</h2>
		...
		<div class="filter">
	 	...
	 	<div class="filter">
			...
			<select name="sales" id="sel_sales">
				...
				<option value="14">2 week sales</option>
				<option value="7">1 week sales</option>
			</select>
			</br>
			<label>From/To :</label>
	...
	<table class="datagrid" width="100%">
		<thead>
			<tr>
				...
				<th>PO Action  <a class="select_all">Select All<input type="checkbox" class="all"></a></th>
			</tr>
		</thead>
		<tbody>
		</tbody>
...
$(function(){
	...
	product_list(0,0,0,0,0,0,60,'all',0);
})
po_product_arr=[];	
pid_arr=[];
...
$('#sel_cat').live('change',function(){
	...
	product_list(menu,cat,brand,vendor,terr,state,days,type,start);
...
function product_list(menu,cat,brand,vendor,terr,state,days,type,start)
{
	$.post('<?=site_url('admin/jx_top_sold_byqty')?>',{menu:menu,cat:cat,brand:brand,vendor:vendor,terr:terr,state:state,days:days,type:type,sales_type:'new',start:start},function(data){
		if(data.prd_list)
		{
			html='';
			k=1;
			...
			$.each(data.prd_list,function(i,p){
				...
				html +='<tr class="prd_row '+c+'" cur_stk="'+p.stk+'" exp_stk="'+p.exp_stk+'" brandid="'+p.brandid+'" product_id="'+p.product_id+'">';
				html +='<td>'+(k++)+'</td>';
...
function print_products_summary(type)
{
	...
	if(!brandid)
		brandid=0;
	window.open(site_url+'/admin/print_top_product_summary/'+menuid+'/'+catid+'/'+brandid+'/'+type+'/'+terrid+'/'+stateid+'/'+days+'/'+'new'+'/'+menu_type+'/'+start);
}
