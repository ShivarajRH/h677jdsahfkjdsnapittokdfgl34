...
<?php if($type==4){?>
<td>
        <a href="javascript:void(0)" onclick='act_rec(<?=$r['receipt_id']?>)' class="button button-tiny button-primary">Realize</a> &nbsp; &nbsp;
        <a href="javascript:void(0)" onclick='can_rec(<?=$r['receipt_id']?>)' class="button button-tiny button-caution">Cancel</a> &nbsp; &nbsp;
<!--        <a href="javascript:void(0)" onclick='reconcile_receipt(this,<?=$r['receipt_id']?>)' class="button button-tiny button-action">Reconcile</a>&nbsp; &nbsp;-->
</td>
...
<?php if(($r['is_submitted']==1 && $r['status']==0)|| ($r['activated_on']==0)){?>
        <a href="javascript:void(0)" onclick='act_rec(<?=$r['receipt_id']?>)' class="button button-tiny button-primary">Realize</a> &nbsp; &nbsp;
        <a href="javascript:void(0)" onclick='cancel_onprocessd(<?=$r['receipt_id']?>)' class="button button-tiny button-caution">Cancel</a>&nbsp; &nbsp;
<?php if ($r['is_deposited']==0){?>
        <a href="javascript:void(0)" onclick="upload_depositslip(<?=$r['receipt_id']?>)" class="button button-tiny button-flat">Upload</a><?php }else{?> 
        <a href="javascript:void(0)" onclick="view_uploaddetails(<?=$r['receipt_id']?>)" class="button button-tiny button-flat">View Upload</a>&nbsp;&nbsp;<?php }?>
<?php } ?>
</td>
</tr>
<?php }?>
...
<thead><th>Receipt Details</th><th>Franchisee Name</th><th>Payment Deatils</th><th>Processed Details</th><th>Realized Details</th><!-- <th>Remarks</th><th>Realized On</th><th>Realized By</th> --></thead>
<tbody>
<?php foreach($receipts as $r){?>
<tr class="fr_receipt_det" fr_id="<?php echo $r['franchise_id']?>">
<td>
    ...
    <form id="dl_submit_reconcile_form">
        <table class="datagrid1" width="100%">
            ...
            <tr><td width="150">Unreconcile Amount</td><th>
                    Rs. <input type="text" readonly='true' id="dg_i_unreconciled_value" name="dg_i_unreconciled_value" value="" size="6" class="dg_i_unreconciled_value inp money"/>
                <input type="hidden" readonly='true' size="5" id="franchise_id" name="franchise_id" value="" size="6" class="inp"/></th></tr>
        </table>
        <div>&nbsp;</div>
...
/*=============================< Reconcilation Code Starts here >================================*/
/**====================< RECONCILE ACTION STARTS >========================*/
...
$("#dlg_unreconcile_form").dialog({
    ...
    ,buttons:{
        "Reconcile":function() {
            ...
            var dlg = $(this); //$("#dlg_unreconcile_form")
...
function load_unconciled_invoices(invs_sel,franchise_id) {
...
function load_unconciled_debit_notes(invs_sel,franchise_id) {
    //on click
    var invs = "<option value='00'>Choose</option>\n";
...
function dg_recon_change_document_type(elt,sel_invoices,row_name,parent,dlgname) {
        ...
        if(document_type == 'inv') {
            ...
            load_unconciled_debit_notes(invs_id,franchise_id);
        }
    ...
    $.post(site_url+"/admin/jx_receipt_reconcile_list/"+receipt_id+"/"+franchise_id,{},function(resp) {
        if(resp.status == 'success') {
            recon_list += "<h3>View reconciled list for Receipt #"+receipt_id+"</h3>\n\
                            <table class='datagrid1' cellpadding='4' cellspacing='1' width='40%'>\n\
                                <tr><td>Receipt #</td><th align='left'>"+resp.receipt_det.receipt_id+"</th></tr>\n\
                                <tr><td>Receipt Amount</td><th align='left'>Rs. "+format_number(resp.receipt_det.receipt_amount)+"</th></tr>\n\
                                <tr><td>Un reconciled Amount</td><th align='left'>Rs. "+format_number(resp.receipt_det.unreconciled_value)+" </th></tr>\n\
                                <tr><td>Created On</td><th align='left'>"+resp.receipt_det.created_date+"</th></tr></table>";
...
.error_status,.dg_error_status { font-size: 12px; color:#B44F4F; }

.datagrid tfoot td { text-align:inherit;}
.money { text-align: left; width: 78px; }
.center {
    text-align: center;
}
/*==================< RECONCILATION STYLE ENDS >==================*/
</style>
