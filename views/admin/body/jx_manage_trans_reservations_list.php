<?php
$output = $cond = $cond_batch = $inner_loop_cond = $re_allot_all_block=$orderby_cond = $datefilter_msg =$msg_generate_pick_list=$block_alloted_status=$batch_btn_link=$from=$to='';
...
else 
{
            ...
            //$transid = $arr_trans['transid']; //from_unixtime(tr.init,'%D %M %Y') as str_date
            $sql="select * from (
...
#Other functions
$resonse2='';
if(isset($fil_menulist) && count($fil_menulist) && $menuid==0) {
    asort($fil_menulist);
...
if(isset($fil_brandlist) && count($fil_brandlist) && $brandid==0) {
    asort($fil_brandlist);
...
if(isset($fil_territorylist) && count($fil_territorylist) && $terrid==0) {
...
if(isset($fil_townlist) && count($fil_townlist) && $townid==0) {
    asort($fil_townlist);
...
if(isset($fil_franchiselist) && count($fil_franchiselist) && $franchiseid==0) {
