		...
		<tbody>
			...
			<td><input type="text" class="inp prod_mrp" name="mrp%pid%[]" size=5 pmrp="%mrp%" value="%mrp%">
				<div class="upd_pmrp_blk" >Update Deal and product MRP 
					<input type="checkbox" value="1" class="upd_pmrp" name="upd_pmrp%pid%[]" >
				</div>
			</td>
			<td>%price%</td>
			<td>
				<input type="text" class="inp pprice" name="price%pid%[]" size=5 value="%price%">
			</td>
			<td>
...
$(function(){
	...
	$("#srch_barcode").keyup(function(e){
		if(e.which==13)
		{
			...
			$(".barcode"+$(this).val()).removeClass("unscanned");
	...
	$('#apply_grn_form .prod_mrp').live('keyup',function(){
		...
		var upd_mrp_blk = $(this).parent().find('.upd_pmrp_blk');
			...
			$('input[name="upd_pmrp"]',upd_mrp_blk).attr('checked',false);
	...
	$("#abd_barcode").keydown(function(e){
		if(e.which==13)
		{
			...
			*/
			...
			$('.pbcodecls'+chk_prodid).parents('tr.unscanned').removeClass('unscanned');
...
function remove_imei(imei,pid)
{
	...
	{
		h=h+'<span onclick="remove_imei(\''+imeis[i]+'\','+pid+')" style="cursor:pointer;">'+imeis[i]+'</span>';
	}
